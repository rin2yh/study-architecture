...
トレースの起点は store BFF（React Router の本番サーバ）まで引き上げてある。ブラウザ要求 →
BFF → edge-proxy → Go 各サービスが 1 本のトレースになり、Go 各サービスへ `traceparent` が
伝播する。BFF の計装は `client/instrument.mjs` を `node --import` で先読みして有効化し、OTLP
宛先 env が無い構成（backoffice 等）では no-op になる。BFF ログの trace 相関は未導入
（BFF は独自の構造化ログを持たず、アクセスログのみのため見送り。必要になった時点で追加する）。
