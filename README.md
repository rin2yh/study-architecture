...
`mise up:obs` 直後、Grafana に固定ダッシュボードが provisioning 済みで開ける（JSON は
`infra/o11y/dashboards/`、手動作成に依存しない）。メトリクス名は Prometheus の OTLP 変換後
（`UnderscoreEscapingWithSuffixes`）を実測して配線している。

- **RED — サービス別 Rate / Error / Duration**: サービス別の req/s・5xx 率・レイテンシ
  (p50/p95/p99)、ルート別 / ステータスコード別。`$service` 変数で絞れる。
- **DB プール — pgxpool 統計**: コネクション (total/acquired/idle/max)・取得レート・空プール待ち
  (飽和の兆候)・取得所要時間（otelpgx `RecordStats`、Step 4-5）。
