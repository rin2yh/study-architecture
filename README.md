...
配送 (shipment) は決済確定を起点に非同期で手配する。payment が確定 (`paid`/`settled`/`captured`) で
`payment.settled` を broker (Redis Streams) に publish し、shipping が購読して preparing 枠を作る
（[doc/adr/202606211200](doc/adr/202606211200-event-driven-shipment-on-payment-settled.md)）。
