...
テストは原則 `foo.go` ↔ `foo_test.go` のように **実装ファイルと 1:1 で対応** させる。
`helper_test.go` のような **実装ファイルと対にならないテスト専用ファイルは作らない**。

- 複数のテストファイルで共有するヘルパー (seed 関数、`dbEnv` などの定数) は、対になる
  いずれかの `*_test.go` に同梱する。例: `query.go` / `command.go` に分かれたパッケージなら、
  読み取り側が前提データを seed する流れに沿って `query_test.go` に `seedXxx` を置き、
  同パッケージの `command_test.go` からも使う。
- 肥大化した、あるいは複数パッケージで共有したくなったら、その時点で **専用パッケージに
  切り出す** (例: 既存の `server/internal/test/db` のような共有テスト支援パッケージ)。

理由: 「どの実装に対するテストか」がファイル名で対応付くと、変更時に見るべきテストが
一目で分かる。対応関係のない `*_test.go` が増えると、ヘルパーの置き場所が場当たり的になり、
どこに何があるか追いづらくなる。対の test ファイルに置くか、明確に独立した支援パッケージに
するか、どちらかに寄せる。
