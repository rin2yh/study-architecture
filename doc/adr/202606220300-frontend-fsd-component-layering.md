...
- Status: Accepted (entities の例から `session` は ADR-[[202606231400]] で `features/auth` に移動。`cart` のみ残置)
- Date: 2026-06-22
...
- `pages` 層 = `routes/`。各ルートは `routes/<page>/` フォルダで表し、**route モジュール
  (`routes/<page>/route.tsx`) がページそのもの**で、`loader` / `action` / default /
  `ErrorBoundary` / `HydrateFallback` を持つ (フレームワーク契約はページの責務)。ファイル名を
  `route.tsx` で固定するのは、`home/home.tsx` のようなフォルダ名との重複を避けるため
  (テストは同階層の `route.test.tsx`)。そのページ専用の表示コンポーネントは
  `routes/<page>/components/` に**コロケート**する
