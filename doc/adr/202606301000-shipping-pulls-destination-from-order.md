...
- settled イベントは payment のアウトボックスから発行される (ADR-[[202606300600]])。宛先 (宛名・住所) は PII であり、payload に載せると payment 行とブローカのトピック (`payment-events`) に PII が残る。宛先を必要としない 2 か所に PII を広げることになる。
...
- 決め手: PII を必要としない payment とブローカに残さない (ADR-[[202606250141]] の「秘匿情報は必要な所だけ」)。
...
- **settled イベントに宛先を載せ payment が中継する** (本 ADR 初稿): shipping→order の同期依存を作らずイベントだけで完結できるが、宛先 (PII) が `payment.payments` とブローカに残る。秘匿情報を必要な所だけに留める方針 (ADR-[[202606250141]]) と緊張するため差し替えた。consumer 消費時の取得 1 回は、PII の拡散を避ける対価として許容する。
