...
- **トークンを UUID にする**: UUIDv4 は乱数部 122bit で総当たりは不能だが、256bit 乱数トークン
  ...
  保存」を維持する。
- **専用の auth/identity サービスを新設**: 認証を 1 か所に集約できるが、`password_hash`・
  セッションは「会員という identity」の一部で member 集約と密結合。別サービスに割ると 1 つの
  集約が 2 サービスに分かれ、ログインのたびに会員データとの突き合わせ＝サービス間結合・整合性
  問題を招く（集約の分割はアンチパターン）。題材の「サービスベースアーキテクチャ」は粗粒度の
  ドメインサービスを少数置く方式で、member が自分の認証を持つ方が方式に合う。Step 0 での
  サービス追加も早すぎる複雑化（[[0001]] のロードマップ）。
  - 将来分けて効くのは認証情報の**保管**ではなく、**セッション検証と認可の強制点**（`X-Member-Id`
    を誰が付けてよいかの信頼境界）。これは新ドメインサービスではなく既存の edge-proxy / Step 1 の
    BFF 層へ寄せるのが筋。すなわち **credential/session の所有は member のまま、検証・強制点は
    将来 edge/BFF へ**。専用 identity サービスが正当化されるのは SSO / OAuth / 外部 IdP / 複数
    認証元を束ねる段で、単一会員基盤の現状では当てはまらない。
