...
- stub (`internal/stub`) は 4 メソッドを 1 つの `RDB` に実装したまま据え置く (型名は
  `Repo` から `rdb` 層を fake する意図で `RDB` に揃えた)。構造的部分型により `RDB` は
  `handler.Query` と `handler.Command` の両方を満たすので、テストでは同じ stub を
  query / command の両引数に渡せる。
