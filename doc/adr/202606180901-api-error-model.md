...
`code` の値はこの `middleware` のソース (`ErrorJSON` のマッピングと `AppError` コンストラクタ群)
だけを SSOT とする。各サービスの OpenAPI は `code: string` とだけ定義して値を列挙しない
(yaml や本 ADR に一覧を書くと追加のたびに全箇所を直す羽目になり、`NewError` の任意コードも
拾えないため)。上の一覧は本 ADR 制定時点の例示で、増減はソースを正とする。
