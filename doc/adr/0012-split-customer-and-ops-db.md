...
- profile 無し (default): networks / volumes / DB / backend / migrate anchor / edge-proxy
- `external` profile: 顧客 UI (`store`, `mypage`)
- `internal` profile: 運用 UI (`backoffice`)
- `tools` profile: migration ワンショットジョブ (`migrate-<svc>`)
