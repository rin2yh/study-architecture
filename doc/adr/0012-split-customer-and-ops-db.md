...
### Network topology (internal can reach external, not vice versa)

DB 分離だけでなく **Docker network レベルでも経路を分け** て「社外 → 社内」アクセスを
落とす。

- **external network (社外公開)**: 顧客 UI (store/mypage)、顧客が呼ぶ全サービス
  (product / order / payment / member / shipping)、db-customer
- **internal network (社内専用)**: 運用 UI (backoffice)、全サービス、db-customer、db-ops

各サービスは next の network 帰属を持つ。
...
- 社外 (store) → 社内 (db-ops, backoffice): **名前解決不可** (DNS が internal scope のみ)。
- 社内 (backoffice) → 社外 (order 等): backoffice 自身は internal にしか居ないが、
  社外サービスが両 network に attach されているため internal 経由で到達できる。
- db-customer は社内からも書き込みアクセス可能だが、これは ADR で許容した「運用照会」用途。
  権限分離は Postgres の role 設計で別途行う想定 (将来の Issue)。
