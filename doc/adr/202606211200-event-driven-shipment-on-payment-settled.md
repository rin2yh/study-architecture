...
あり、両 private network に足を持つ。ADR-[[202606170909]] は**業務サービス**を multi-home させず
edge-proxy / backoffice の 2 proxy に寄せる方針だが、broker は DB と同類の**共有 infra** で
業務サービスではない。位置づけとしては、edge-proxy が同期の境界越えを担うのと対に、broker が
顧客系→運用系を結ぶ非同期チャネルを担う (payment は publish のみ・shipping は consume のみ)。
