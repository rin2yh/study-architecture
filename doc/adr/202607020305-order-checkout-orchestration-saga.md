...
- Status: Accepted
- Date: 2026-07-02
- Relates to: ADR-[[202607011621]] (マイクロサービス移行。本 ADR は具体化タスク 2), ADR-[[202607011720]] (order FSM。本 ADR が語彙を拡張), ADR-[[202606261702]] (order.cancelled 補償コレオグラフィ), ADR-[[202606211200]] (payment.settled イベント), ADR-[[202606261212]] (Outbox), ADR-[[202606261214]] (冪等), ADR-[[202606261216]] (致命/縮退), ADR-[[202606250159]] (traceparent), GitHub #98 (親)
...
- **order.status をサーガ状態として拡張**: タスク 1 の `placed → paid → shipped` / `cancelled` に、中間 `awaiting_payment` と失敗終端 `rejected` を足す (ADR-[[202607011720]] の語彙を追記更新)。
  - 遷移辺: `∅→placed` (受理)、`placed→awaiting_payment` (予約成功購読)、`placed→rejected` (在庫不足購読・補償不要)、`awaiting_payment→paid` (`payment.settled` 購読)、`awaiting_payment→rejected` (決済失敗購読 → 予約解放の補償)、`paid→shipped` (出荷購読)、`{placed,awaiting_payment,paid}→cancelled` (顧客取消。`shipped` からは不可)。
...
- **補償はコレオグラフィを再利用**: 失敗遷移 (`awaiting_payment→rejected` 等) は `order.cancelled` 相当のイベントで inventory 解放などを後追いする (ADR-[[202606261702]])。インライン `abandonCheckout` / `DeleteOrder` は廃止し、失敗も行を残して履歴に記録する。
...
- サーガ状態を order.status に集約するため FSM が太る (`awaiting_payment` / `rejected` 追加)。タスク 1 ADR を追記更新する。
