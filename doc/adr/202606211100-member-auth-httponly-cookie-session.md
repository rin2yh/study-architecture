...
UI（mypage）にログイン/ログアウトの導線を入れ、会員を認証したい。ADR-[[202606170900]] のサービスベース
構成では認証の所有者を 1 つに定める必要がある。会員情報は member ドメインが持つので、
パスワードとセッションも member が所有するのが自然。

ADR-[[202606170905]] で UI のデータ取得は **サーバ側ローダ → 各サービス HTTP 呼び出し**に閉じている。
...
- 既存エンドポイントは path id を `int64`（`IdPath`）に統一していたが、セッションの `{id}` は
  ...
  トークン）**とする。これは ADR-[[202606180903]] までの int64 規約から意図的に外れる箇所。
- エラーモデル（ADR-[[202606180901]]）に **401 `unauthorized`** を追加（`middleware.Unauthorized`）。

### UI（mypage / ADR-[[202606170908]] React Router v7）
...
- **`Secure` 未設定は本番では不適**。公開時は TLS 終端の edge を前提に `Secure` を有効化する。
  この ADR の決定は「ローカル完結・費用ゼロ」（ADR-[[202606170900]]）の制約下での妥協であることを明記する。
...
- **専用の auth/identity サービスを新設**: 認証を 1 か所に集約できるが、`password_hash`・
  ...
  サービス追加も早すぎる複雑化（ADR-[[202606170900]] のロードマップ）。
