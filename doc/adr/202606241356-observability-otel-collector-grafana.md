# ADR-202606241356: 可観測性を OpenTelemetry + Collector + Grafana スタックで構築する

- Status: Accepted
- Date: 2026-06-24
- Relates to: ADR-[[202606170900]] (サービスベース採用), ADR-[[202606240522]] (Step 3 ドメインごと DB 分割), ADR-[[202606211200]] (payment settled → shipping のイベント駆動 / Redis Streams), ADR-[[202606230930]] (store BFF の信頼境界), ADR-[[202606220716]] (CI のデプロイ量子分割)
...
### スタック: OpenTelemetry SDK → otel-collector → Grafana (Tempo / Loki / Prometheus)

計装は **OpenTelemetry**。各サービスは OTLP/gRPC で **otel-collector へ送る**。バックエンドは最初から
**Grafana スタック** (トレース=Tempo、ログ=Loki、メトリクス=Prometheus、可視化=Grafana) を採用する。

4-1 (#60) ではトレースのみ稼働させるため compose に **collector + Tempo + Grafana** を立てる。
4-2 (#61) で Loki、4-3 (#62) で Prometheus を足す。**collector を必ず経由する**ことで、バックエンドを
増やしても各 `main.go` の exporter 向き先 (collector 固定) を触らずに済む。

### メトリクスの取り込みは push (アプリ→collector) と pull (collector→Prometheus) のハイブリッド

メトリクス (4-3 / #62) は **アプリ → collector は OTLP push、collector → Prometheus は scrape (pull)**
の 2 段にする。collector が `/metrics` を公開し、Prometheus がそれを scrape する。

- **アプリ側は push に統一**: トレース・ログと同じ OTLP の 1 本道に乗る。`shipping-worker` のような
  短命・非 HTTP の consumer も送れ、各サービスは collector への egress だけでよく network 分離
  (ADR-[[202606170909]]) と相性が良い。
- **収集の最終段は pull**: Prometheus は OTLP 直接受信ではなく**枯れた scrape**で取り込む。scrape 対象は
  collector 1 つに集約され、DB 分割で増えた各サービスを列挙するサービスディスカバリが要らない。
- **各サービスの死活は scrape で見ない**: 既存の `GET /healthz` と compose / LB のヘルスチェックで見る。
  pull が自動で付与する target-up 監視は使わない判断。collector 自体の死活は scrape 成否で分かる。

### 計装スコープは Go 5 サービス + shipping-worker。BFF (Node) は分離

4-1 は Go 側 (5 サービス + `shipping-worker`) に限定する。store BFF (Node / TanStack Start,
ADR-[[202606230930]]) の計装は OTel エコシステムが別で PR を分けた方が安全なため、別サブイシューに回す。
edge-proxy (nginx) はリクエストヘッダを upstream へ素通しするので `traceparent` は伝播し、Go サービス間は
自動でつながる。nginx 自身に span は持たせない。
...
- **collector 経由のコスト**: compose / CI (ADR-[[202606220716]] の 2 ワークフロー) / `scripts/e2e-up.sh` の
  起動対象に collector + Tempo + Grafana が増える。各 Step でバックエンドを足す手間も発生する。
- **向き先の安定**: アプリの exporter は collector 固定なので、Tempo→他バックエンド差し替えや
  Loki/Prometheus 追加で `main.go` の env を触らずに済む。
...
- **メトリクスは push/pull ハイブリッド**: アプリは OTLP push で統一しつつ Prometheus は collector を
  scrape する。scrape 対象が collector に集約される代わり、各サービスの死活は healthz / LB 側で見る前提になる。
- **BFF は線が途中から**: 4-1 時点でトレースは最初に当たる Go サービス起点になり、ブラウザ→BFF の
  最初のホップは欠ける。BFF 計装の別サブイシューで解消する。
...
- **collector を挟まずアプリから backend へ直結**: ホップが 1 つ減るが、backend 追加・差し替えのたびに
  全サービスの exporter 向き先を変える必要があり、横断変更が頻発する。collector を緩衝にする。
...
- **メトリクスをアプリから Prometheus へ直接 push (remote_write / Prometheus の OTLP receiver)**:
  collector の scrape 段が減り構成は単純になるが、Prometheus 3.0 で GA したばかりの新しめの経路に乗る。
  学習用途では枯れた scrape を選び、collector を Prometheus に scrape させる。
- **各サービスを Prometheus が直接 scrape (純 pull)**: Prometheus の王道だが、全サービスのサービス
  ディスカバリ・到達経路が要り (DB 分割でサービスが多い)、`shipping-worker` のような短命・非 HTTP
  プロセスを取りこぼす。メトリクスだけトレース/ログと別 paradigm になる。
