...
- **append-only の在庫変動台帳を単一情報源にする**。入庫 (`stock_ins`) / 予約 (`reservations`) / 確定 (`confirmations`) / 解放 (`releases`) / 期限切れ回収 (`expirations`) を**種別ごとのテーブルに追記**し (判別列 `kind` や `reason` を持たない)、確定・解放・期限切れは予約 1 件の相互排他な終端状態で各テーブル `reservation_id` 主キーで高々 1 行。
- **導出できる値は行に保存しない**。在庫数カラムを持たず利用可能在庫は集計で導き、予約の期限も `expires_at` 列を持たず `created_at + 固定 TTL` (DB 関数 `inventory.reservation_ttl()`) から導く。状態 (open/confirmed/released/expired) も status 列でなく行の有無と TTL から導出する。
