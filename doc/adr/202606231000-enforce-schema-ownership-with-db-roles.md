...
- **ロール生成と GRANT は goose の外の冪等スクリプト `scripts/grant.sh` 1 本に集約**し、`ec` で
  適用する。GRANT は「スキーマ進化 (一方向 migration)」ではなく「再適用する宣言的ポリシー」なので、
  version 追跡される goose ではなく、何度でも流せる 1 スクリプトで表現する。将来 `ec` が追加する
  table/sequence にも効くよう `ALTER DEFAULT PRIVILEGES FOR ROLE ec` を併用する。
