...
import (
	...
	"github.com/rin2yh/study-service-base-architecture/services/member/internal/db"
)

// MemberRepository は handler が依存する会員データアクセスの境界。
// interface にすることで handler のテストでスタブへ差し替えられる。
type MemberRepository interface {
	ListMembers(ctx context.Context) ([]db.MemberMember, error)
}

// Repository は pgx プール上の sqlc Querier を用いた MemberRepository の実装。
