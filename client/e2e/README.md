...
store / backoffice は別スタックなので、Playwright の project を分けている。`--project` で
起動するスタックを片方に絞れる（store=社外 / backoffice=社内）。`webServer` は `--project` では
絞られないため、config が CLI の `--project` を読んで起動スタックも同じ project に合わせる
（未指定なら両方）。
