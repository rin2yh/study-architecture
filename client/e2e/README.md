...
setup project が `scripts/e2e-up.sh` で DB 起動 → migrate → 対象スタックの build/up（detached）まで
行い、product にシード（store はログイン用 member も）を投入し、frontend は healthcheck を持たない
ため `baseURL` の到達を待ってから tests に渡す。teardown project は `scripts/e2e-down.sh` でスタックを
停止する。app ごとの差分（profile / baseURL / member シードの要否）は `tests/stack/apps.ts` の
`appConfig` に集約し、setup/teardown spec は project 名から対象 app を引いて共有する。シードは冪等で、
store のテストは `/login` 画面からログインする。
