...
func TestCancelConsumerRun(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer cancel()
			stub := &cancellerStub{err: tt.args.err}
			...
			if len(stub.got) != len(tt.want.gotOrderIDs) {
				t.Fatalf("CancelShipmentForOrder called with %v, want %v", stub.got, tt.want.gotOrderIDs)
			}
...
func TestCancelConsumerSubscribesToOrderEvents(t *testing.T) {
	...
	if err := NewCancel(sub, &cancellerStub{}).Run(ctx); !errors.Is(err, context.Canceled) {
		t.Fatalf("Run() = %v, want context.Canceled", err)
	}
