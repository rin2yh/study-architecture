...
type checkoutRepo struct {
	...
	err      error
	gotTotal int64
	gotLines []repository.CheckoutLine
}

func (r *checkoutRepo) Checkout(_ context.Context, memberID int64, status string, total int64, lines []repository.CheckoutLine) (db.OrderOrder, []db.OrderOrderItem, error) {
	r.gotTotal = total
	r.gotLines = lines
	if r.err != nil {
...
func TestCheckoutSnapshotAndTotal(t *testing.T) {
	...
	// 500*2 + 1500*1
	if repo.gotTotal != 2500 {
		t.Fatalf("total = %d, want 2500", repo.gotTotal)
	}
	want := []repository.CheckoutLine{
	...
	if len(repo.gotLines) != len(want) {
		t.Fatalf("lines = %+v, want %+v", repo.gotLines, want)
	}
	for i := range want {
		if repo.gotLines[i] != want[i] {
			t.Fatalf("line[%d] = %+v, want %+v", i, repo.gotLines[i], want[i])
		}
	}
