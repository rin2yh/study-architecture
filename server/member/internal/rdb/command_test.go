...
func TestCreateMember(t *testing.T) {
	...
	seedMembers(t, pool, Member{Email: "exist@example.com", DisplayName: "既存"})

	t.Run("正常系 作成行を返す", func(t *testing.T) {
		got, err := r.CreateMember(t.Context(), MemberCreate{Email: "new@example.com", DisplayName: "新規会員"})
		if err != nil {
	...
	t.Run("準正常系 email 重複は ErrConflict", func(t *testing.T) {
		if _, err := r.CreateMember(t.Context(), MemberCreate{Email: "exist@example.com", DisplayName: "重複"}); !errors.Is(err, dberr.ErrConflict) {
...
func TestUpdateMember(t *testing.T) {
	...
	r := NewMemberCommand(pool)
	seedMembers(t, pool,
		Member{Email: "a@example.com", DisplayName: "会員A"},
		Member{Email: "b@example.com", DisplayName: "会員B"})

	t.Run("正常系 既存行を更新して返す", func(t *testing.T) {
		got, err := r.UpdateMember(t.Context(), MemberUpdate{ID: 1, Email: "a2@example.com", DisplayName: "会員A更新"})
		if err != nil {
	...
	t.Run("準正常系 未存在は ErrNotFound", func(t *testing.T) {
		if _, err := r.UpdateMember(t.Context(), MemberUpdate{ID: 9999, Email: "x@example.com", DisplayName: "x"}); !errors.Is(err, dberr.ErrNotFound) {
			t.Fatalf("err = %v, want ErrNotFound", err)
		}
	})
	t.Run("準正常系 email 重複は ErrConflict", func(t *testing.T) {
		if _, err := r.UpdateMember(t.Context(), MemberUpdate{ID: 1, Email: "b@example.com", DisplayName: "衝突"}); !errors.Is(err, dberr.ErrConflict) {
			t.Fatalf("err = %v, want ErrConflict", err)
		}
	})
}
...
func TestCreateAddress(t *testing.T) {
	...
	t.Run("正常系 作成行を返す", func(t *testing.T) {
		got, err := r.CreateAddress(t.Context(), AddressCreate{
	...
	t.Run("準正常系 未存在 member への追加は FK 違反で ErrNotFound", func(t *testing.T) {
		if _, err := r.CreateAddress(t.Context(), AddressCreate{
...
func TestUpdateAddress(t *testing.T) {
	...
	r := NewMemberCommand(pool)
	memberID := seedOneMember(t, pool, "user@example.com", "h")
	seedAddresses(t, pool, memberID, Address{Recipient: "旧名", PostalCode: "1500001", Prefecture: "東京都", City: "渋谷区", Line1: "旧1-2-3"})

	t.Run("正常系 member 所有の住所を更新して返す", func(t *testing.T) {
		got, err := r.UpdateAddress(t.Context(), AddressUpdate{
	...
	t.Run("準正常系 他 member の住所は更新できず ErrNotFound", func(t *testing.T) {
		if _, err := r.UpdateAddress(t.Context(), AddressUpdate{ID: 1, MemberID: memberID + 1, Recipient: "x", PostalCode: "x", Prefecture: "x", City: "x", Line1: "x"}); !errors.Is(err, dberr.ErrNotFound) {
			t.Fatalf("err = %v, want ErrNotFound", err)
		}
	})
}
...
func TestDeleteAddress(t *testing.T) {
	...
	t.Run("正常系 削除すると取得できなくなる", func(t *testing.T) {
		if err := r.DeleteAddress(t.Context(), AddressRef{ID: 1, MemberID: memberID}); err != nil {
			t.Fatalf("DeleteAddress: %v", err)
		}
		if _, err := q.GetAddress(t.Context(), AddressRef{ID: 1, MemberID: memberID}); !errors.Is(err, dberr.ErrNotFound) {
			t.Fatalf("削除後 GetAddress err = %v, want ErrNotFound", err)
		}
	})
	t.Run("準正常系 未存在の削除は冪等 (error なし)", func(t *testing.T) {
		if err := r.DeleteAddress(t.Context(), AddressRef{ID: 9999, MemberID: memberID}); err != nil {
...
func TestCreateSession(t *testing.T) {
	...
	t.Run("正常系 作成した行を返す", func(t *testing.T) {
		got, err := r.CreateSession(t.Context(), SessionCreate{
			ID: "hash-new", MemberID: memberID, ExpiresAt: expiresIn(time.Hour),
		})
		if err != nil {
			t.Fatalf("CreateSession: %v", err)
		}
		assert.DeepEqual(t, Session{ID: "hash-new", MemberID: memberID}, got, "ExpiresAt", "CreatedAt")
	})
	t.Run("準正常系 同一 id は ErrConflict", func(t *testing.T) {
		p := SessionCreate{ID: "hash-dup", MemberID: memberID, ExpiresAt: expiresIn(time.Hour)}
...
func TestDeleteSession(t *testing.T) {
	...
	q := NewMemberQuery(pool)
	memberID := seedOneMember(t, pool, "user@example.com", "stored-hash")
	if _, err := r.CreateSession(t.Context(), SessionCreate{
