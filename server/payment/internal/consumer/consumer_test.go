...
func newTestConsumer(t *testing.T, refunder PaymentRefunder) (*Consumer, *miniredis.Miniredis) {
	t.Helper()
	mr := miniredis.RunT(t)
...
func TestReadAndProcess(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"準正常系 不正な orderId は返金せず ack せず pending に残す",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c, _ := newTestConsumer(t, refunder)
			ctx := t.Context()
			if err := c.ensureGroup(ctx); err != nil {
			...
			p, err := c.rdb.XPending(ctx, orderevent.Stream, consumerGroup).Result()
			if err != nil {
...
func TestReadAndProcessClaimsStalePending(t *testing.T) {
	refunder := &refunderStub{err: errors.New("db down")}
	c, mr := newTestConsumer(t, refunder)
	...
	if len(refunder.got) != 2 {
		t.Fatalf("refunder calls = %d, want 2 (1 周目の失敗 + 引き取り後の再処理)", len(refunder.got))
	}
	p, err := c.rdb.XPending(ctx, orderevent.Stream, consumerGroup).Result()
	if err != nil {
		t.Fatalf("XPending: %v", err)
